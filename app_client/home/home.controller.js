(() => {
  ...
  angular.module('trelloClone').controller('homeCtrl', homeCtrl);
  ...
  function homeCtrl(
    ...
    (vm.getBoardData = () => {
      boardData
        .board()
        .then(boardInfo => {
          const { color, lists, _id } = boardInfo.data[0];
          vm.boardColor = color;
          vm.lists = lists;
          vm.boardID = _id;
        })
    ...
    const foundListIndex = (listId, callback) => {
    ...
    const foundTaskIndex = (taskId, callback) => {
      for (let i = 0; i < vm.lists.length; i += 1) {
        for (let j = 0; j < vm.lists[i].tasks.length; j += 1) {
          // eslint-disable-next-line no-underscore-dangle
    ...
    const checkListPosition = (listIndex, list) => {
      return new Promise((resolve, reject) => {
        ...
        if (list._id !== vm.lists[listIndex]._id) {
          ...
          if (
            vm.lists[listIndex] === undefined ||
            vm.lists[listIndex]._id !== list._id
          ) {
            reject({ message: 'list not found' });
          }
        }
    ...
    const checkTaskPosition = (listIndex, taskIndex, list, task) => {
      return new Promise((resolve, reject) => {
        if (!task._id) {
        ...
        if (!vm.lists[listIndex] || !vm.lists[listIndex].tasks[taskIndex]) {
          ...
          if (
            !vm.lists[listIndex].tasks[taskIndex] ||
            !vm.lists[listIndex].tasks[taskIndex]._id
          ) {
            reject({ message: 'task not found' });
          }
        }

        const taskIdForEdit = vm.lists[listIndex].tasks[taskIndex]._id;
        if (taskId !== taskIdForEdit) {
          ...
          if (
            vm.lists[listIndex].tasks[taskIndex] === undefined ||
            vm.lists[listIndex].tasks[taskIndex]._id !== taskId
          ) {
            reject({ message: 'task not found' });
          }
        }
    ...
    vm.updateBoard = (color, lists) => {
      ...
      const boardDataForUpdate = { lists, color };

      // send request to boardData.service, updateBoard function
      boardData
        .updateBoard(vm.boardID, boardDataForUpdate)
        .then(newBoardData => {
          const { color, lists } = newBoardData.data;
          vm.boardColor = color;
          vm.lists = lists;

          // send board information to another users
          socketioFactory.emit('boardServer', {
            boardColor: vm.boardColor,
            lists: vm.lists,
          });
        })
        .catch(err => {
          if (err.status === 401) {
            messageFactory.message =
              'Войдите в учётную запись для продолжения.';
            vm.logout();
          } else {
    ...
    vm.createList = listName => {
      ...
      const nameForCreate = { listName };

      // send request to listData.service, createList function
      listData
        .createList(vm.boardID, nameForCreate)
        .then(listData => {
    ...
    vm.updateList = (list, listIndex, newListName) => {
      ...
      const nameForUpdate = { listName: newListName };
      ...
      checkListPosition(listIndex, list)
        .then(data => {
          ...
          listData
            .updateList(vm.boardID, listid, nameForUpdate)
            .then(newList => {
    ...
    vm.createTask = (list, index, taskName, side) => {
      ...
      const taskNameForCreate = { taskName, side };
      ...
      taskData
        .createTask(vm.boardID, listid, taskNameForCreate)
        .then(taskData => {
          if (side === 'top') {
    ...
    vm.updateTaskPosition = (
      ...
      checkTaskPosition(thisListIndex, thisTaskIndex, list, task)
        .then(data => {
          ...
          const thisTask = vm.lists[thisListIndex].tasks.splice(thisTaskIndex, 1);
    ...
    vm.updateTask = (list, task, newTaskData) => {
      ...
      taskData
        .updateTask(vm.boardID, list._id, task._id, newTaskData)
        .then(newTaskData => {
    ...
    vm.dateTimePicker = (listIndex, taskIndex, list, task) => {
      ...
      checkTaskPosition(listIndex, taskIndex, list, task)
        .then(data => {
          listIndex = data.listIndex;
          taskIndex = data.taskIndex;
          const newDueDate = vm.now;
            const date = vm.taskDate;
            const time = vm.taskTime;
          if (date !== null) {
          ...
          const listForUpdate = vm.lists[listIndex];
            const taskForUpdate = listForUpdate.tasks[taskIndex];
    ...
    vm.deleteTask = (list, listIndex, task, taskIndex) => {
      ...
      checkTaskPosition(listIndex, taskIndex, list, task)
        .then(data => {
          listIndex = data.listIndex;
          taskIndex = data.taskIndex;
          const listid = vm.lists[listIndex]._id;
            const taskid = vm.lists[listIndex].tasks[taskIndex]._id;
          taskData
            .deleteTask(vm.boardID, listid, taskid)
            .then(() => {
    ...
    vm.uploadAttachment = (evt, listIndex, taskIndex, list, task) => {
      ...
      checkTaskPosition(listIndex, taskIndex, list, task)
        .then(data => {
          listIndex = data.listIndex;
          taskIndex = data.taskIndex;
          const inputElement = angular.element(document.querySelector('#file'));
            const inputElementMobile = angular.element(
              document.querySelector('#fileMobile')
            );
          let file;
          ...
          const {listName} = vm.lists[listIndex];
            const listid = vm.lists[listIndex]._id;
            const {taskName} = vm.lists[listIndex].tasks[taskIndex];
            const taskid = vm.lists[listIndex].tasks[taskIndex]._id;
          ...
          inputElementMobile.replaceWith(
          ...
          attachmentData
            .addAttachment(vm.boardID, listid, taskid, file, listName, taskName)
            .then(attachmentData => {
              const { message } = attachmentData.data;
              ...
              switch (message) {
                ...
                case 'FILE_TYPE':
                  vm.fileUploaderMessage =
                    'Допустимый формат файла jpeg, bmp, png.';
                  break;
                ...
                default:
                  ...
                  vm.lists[listIndex].tasks[taskIndex].attachments.push(
                    attachmentData.data
                  );
                  vm.modalTask.attachments =
                    vm.lists[listIndex].tasks[taskIndex].attachments;
                  socketioFactory.emit('modalTaskServer', {
                    modalList: vm.modalList,
                    modalListIndex: vm.modalListIndex,
                    modalTask: vm.modalTask,
    ...
    vm.deleteAttachment = (
      ...
      checkTaskPosition(listIndex, taskIndex, list, task)
        .then(data => {
          ...
          if (
            !vm.lists[listIndex].tasks[taskIndex].attachments ||
            !vm.lists[listIndex].tasks[taskIndex].attachments[attachmentIndex]
          ) {
            return;
          }
          ...
          if (attachmentid !== attachment._id) {
            for (
              let i = 0;
              i < vm.lists[listIndex].tasks[taskIndex].attachments;
              i++
            ) {
              if (
                vm.lists[listIndex].tasks[taskIndex].attachments[i]._id ===
                attachment._id
              ) {
                attachmentid =
                  vm.lists[listIndex].tasks[taskIndex].attachments[i]._id;
          ...
          attachmentData
            .deleteAttachment(vm.boardID, listid, taskid, attachmentid)
            .then(() => {
              vm.lists[listIndex].tasks[taskIndex].attachments.splice(
                attachmentIndex,
                1
              );
              vm.modalTask.attachments =
    ...
    vm.taskEditor = (evt, ...taskData) => {
      const arrForCoords = evt.composedPath();
      ...
      const listContentY = arrForCoords[3].offsetTop;
        const listContentX = arrForCoords[3].offsetLeft;
        const taskY = arrForCoords[1].offsetTop;
        const taskX = arrForCoords[1].offsetLeft;
        const scrollY = arrForCoords[3].scrollTop;
        const scrollX = arrForCoords[7].scrollLeft;
      ...
      vm.editTaskDatePickerVerticalSide =
        vm.taskEditCoordY + 300 > vm.deviceHeight;

      vm.editTaskBtnSide
        ? (vm.editTaskMenuLeft = 'quick-task-edit-buttons-left')
        : (vm.editTaskMenuLeft = '');
      vm.editTaskBtnSide
        ? (vm.taskPositionMenu = 'task-position-menu-left')
        : (vm.taskPositionMenu = '');
      vm.editTaskBtnVerticalSide
        ? (vm.taskVerticalPositionMenu = 'task-position-menu-top')
        : (vm.taskVerticalPositionMenu = '');
      vm.editTaskDatePickerVerticalSide
        ? (vm.taskVerticalPositionDatePicker =
            'quick-task-edit-date-picker-menu-top')
        : (vm.taskVerticalPositionDatePicker = '');
    ...
    vm.listMenuPosition = evt => {
      ...
      const deviceWidth = arrForCoords[7].clientWidth;
        const elementPosition = evt.x;
      elementPosition + 350 > deviceWidth && elementPosition - 350 < 0
        ? (vm.listPositionMenu = 'list-menu-center')
        : elementPosition + 350 > deviceWidth
        ? (vm.listPositionMenu = 'list-menu-left')
        : (vm.listPositionMenu = '');
    };
    ...
    vm.dateSetting = (listIndex, taskIndex, list, task) => {
      ...
      checkTaskPosition(listIndex, taskIndex, list, task)
        .then(data => {
          ...
          // eslint-disable-next-line no-param-reassign
          taskIndex = data.taskIndex;
          const {date} = vm.lists[listIndex].tasks[taskIndex];
          if (date) {
